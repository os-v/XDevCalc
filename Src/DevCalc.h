//
//  @file DevCalc.h
//  @author Sergii Oryshchenko <sergii.orishchenko@gmail.com>
//  @see https://github.com/os-v/XDevCalc/
//
//  Created on 14.03.22.
//  Copyright 2022 Sergii Oryshchenko. All rights reserved.
//

#define PRODUCT_VERSION			1,2,0,0
#define PRODUCT_SVERSION		"1.2.0"

#define PRODUCT_URL			"https://github.com/os-v/XDevCalc"
#define PRODUCT_PRODUCTNAME		"CrossPlatform Developer Calculator [XDevCalc]"
#define PRODUCT_COMPANYNAME		"OS-V"
#define PRODUCT_LEGALCOPYRIGHT		"Copyright (C) Sergii Oryshchenko 2022 " PRODUCT_URL
#define PRODUCT_LEGALTRADEMARKS		"https://github.com/os-v"


